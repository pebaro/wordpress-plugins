<table id="iupdm-rt-denials-data" cellspacing="0" cellpadding="0">
	...
	<tbody>
		<?php
		$deniedRequestsQuery 		= $wpdb->get_results(
			"SELECT * FROM " . $wpdb->prefix . "dm_requests WHERE request_status='denied'"
		);
		// $deniedRequestsQuery 		= "SELECT * FROM " . $wpdb->prefix . "dm_requests WHERE request_status='denied'";
		// $totalDeniedRequestsQuery 	= "SELECT COUNT(1) FROM (${deniedRequestsQuery}) AS combined_table";
		// $totalDeniedRequests 		= $wpdb->get_var( $totalDeniedRequestsQuery );
		// $deniedRequestsPerPage 		= 2;
		// $deniedRequestsPage 		= isset( $_GET[ 'cpage' ] ) ? abs( ( int ) $_GET[ 'cpage' ] ) : 1;
		// $deniedRequestsOffset 		= ( $deniedRequestsPage * $deniedRequestsPerPage ) - $deniedRequestsPerPage;
		// $deniedRequestsResult 		= $wpdb->get_results( $deniedRequestsQuery . " ORDER BY timestamp DESC LIMIT ${deniedRequestsOffset}, ${deniedRequestsPerPage}" );
		// $totalDeniedRequestsPages 	= ceil( $totalDeniedRequests / $deniedRequestsPerPage );

		// $deniedRequestsPageHTML 	= '';

		// if ( $totalDeniedRequestsPages > 1 ){
		// 	$deniedRequestsPageHTML = '<div id="iupdm-denied-requests-pagination"><span>Requests Page ' . $deniedRequestsPage . ' of ' . $totalDeniedRequestsPages . '</span> &nbsp;&nbsp; ' . paginate_links( 
		// 		array(
		// 			'base' 			=> add_query_arg( 'cpage', '%#%' ),
		// 			'format' 		=> '',
		// 			'prev_text' 	=> __('&laquo;'),
		// 			'next_text' 	=> __('&raquo;'),
		// 			'total' 		=> $totalDeniedRequestsPages,
		// 			'current' 		=> $deniedRequestsPage
		// 		)
		// 	) . '</div>';
		// }
		// echo $deniedRequestsPageHTML;
		?>
		<div id="iupdm-denied-requests-pagination">
			<span>Current Timestamp</span> &nbsp; &nbsp; 
			<?php echo date( 'd/m/Y  H:i:s', time() ); ?>
		</div>
		
		<?php
		foreach ( $deniedRequestsQuery as $result ) :
			$request_info = [];
			...
			$actions_select .= 			'<option value="history">Move To Archive</option>';
