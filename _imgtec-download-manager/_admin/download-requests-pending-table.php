<table id="iupdm-rt-pending-data" cellspacing="0" cellpadding="0">
	<thead>
		<tr>
	...
	<tbody>
		...
		$pendingRequestsQuery = $wpdb->get_results(
			"SELECT * FROM " . $wpdb->prefix . "dm_requests WHERE request_status='pending'"
		);
		// $pendingRequestsQuery 		= "SELECT * FROM " . $wpdb->prefix . "dm_requests WHERE request_status='pending'";
		...
		// $requestsPendingResult 	= $wpdb->get_results( $pendingRequestsQuery . " ORDER BY timestamp DESC LIMIT ${requestsPendingOffset}, ${requestsPendingPerPage}" );
		...
		// 	$requestsPendingPageHTML = '<div id="iupdm-pending-requests-pagination"><span>Requests Page ' . $requestsPendingPage . ' of ' . $totalPendingRequestsPages . '</span> &nbsp;&nbsp; ' . paginate_links( 
		// 		array(
		...
		foreach ( $pendingRequestsQuery as $result ) :
			...
			echo 	'<td class="">' . imgtec_font_formatted_strings( $result->download_title, 'sentence' );
