 ...
 * Description: Live search with search saving functionality built with javascript and the WP Rest API.
 * Version: 4.6
 * Author: Rob Masters
...
if ( ! function_exists( 'it_acf_pro_plugin_failure' ) ){
    function it_acf_pro_plugin_failure(){
        ...
        if ( it_is_acf_pro_installed() ){
            ...
            $acf_message  = '<p>' . __( 'You need to ACTIVATE the Advanced Custom Fields Pro plugin for ImgTec Live Search to work fully', LIVESEARCH_TEXTDOMAIN ) . '</p>';
            $acf_message .= '<p>' . sprintf( '<a href="%s" class="button-primary">%s</a>', $acf_activation_url, __( 'Activate Advanced Custom Fields Pro', LIVESEARCH_TEXTDOMAIN ) ) ;
            ...
            $acf_message  = '<p>' . __( 'You need to INSTALL the Advanced Custom Fields Pro plugin for ImgTec Live Search to work fully', LIVESEARCH_TEXTDOMAIN ) . '</p>';
            $acf_message .= '<p>' . sprintf( '<a href="%s" class="button-primary">%s</a>', $acf_install_url, __( 'Install Advanced Custom Fields Pro', LIVESEARCH_TEXTDOMAIN ) ) . '</p>';
        }
...
if ( ! function_exists( 'it__imgtec_required_plugin_failure' ) ){
    function it__imgtec_required_plugin_failure(){
        ...
        if ( it_is_acf_pro_installed() ){
            ...
            $it_required_message  = '<p>' . __( 'You need to ACTIVATE the ImgTec Required plugin for ImgTec Live Search to work fully', LIVESEARCH_TEXTDOMAIN ) . '</p>';
            $it_required_message .= '<p>' . sprintf( '<a href="%s" class="button-primary">%s</a>', $it_required_activation_url, __( 'Activate ImgTec Required', LIVESEARCH_TEXTDOMAIN ) ) ;
            ...
            $it_required_message  = '<p>' . __( 'You need to INSTALL the ImgTec Required plugin for ImgTec Live Search to work fully', LIVESEARCH_TEXTDOMAIN ) . '</p>';
            $it_required_message .= '<p>' . sprintf( '<a href="%s" class="button-primary">%s</a>', $it_required_install_url, __( 'Install ImgTec Required', LIVESEARCH_TEXTDOMAIN ) ) . '</p>';
        }
