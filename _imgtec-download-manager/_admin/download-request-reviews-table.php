<table id="iupdm-rt-reviews-data" cellspacing="0" cellpadding="0">
	<thead>
		<tr>
			<th style="width:7%;padding-left:5px;">IP Address</th>
			<th style="width:8%;">Sent For Review</th>
	...
	<tbody>
		<?php
		$requestsReviewQuery 		= $wpdb->get_results(
			"SELECT * FROM " . $wpdb->prefix . "dm_requests WHERE request_status='review'"
		);
		// $requestsQuery 		= "SELECT * FROM " . $wpdb->prefix . "dm_requests WHERE request_status='review'";
		...
		// 	$requestsPageHTML = '<div id="iupdm-review-requests-pagination"><span>Requests Page ' . $requestsPage . ' of ' . $totalRequestsPages . '</span> &nbsp;&nbsp; ' . paginate_links( 
		// 		array(
		...
		foreach ( $requestsReviewQuery as $result ) :
			$request_info = [];
