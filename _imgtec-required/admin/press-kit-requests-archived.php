<table id="itpk-rt-pending-data" cellspacing="0" cellpadding="0">
	...
	<tbody>
		<?php
		$archivedRequestsQuery = $wpdb->get_results(
			"SELECT * FROM " . $wpdb->prefix . "presskit_requests WHERE kit_request='archived'"
		);
		// $archivedRequestsQuery 		= "SELECT * FROM " . $wpdb->prefix . "presskit_requests WHERE kit_request='pending'";
		// $totalarchivedRequestsQuery = "SELECT COUNT(1) FROM (${archivedRequestsQuery}) AS combined_table";
		// $totalPendingRequests 		= $wpdb->get_var( $totalarchivedRequestsQuery );
		...
		// $requestsPendingResult 	= $wpdb->get_results( $archivedRequestsQuery . " ORDER BY timestamp DESC LIMIT ${requestsPendingOffset}, ${requestsPendingPerPage}" );
		...
		foreach ( $archivedRequestsQuery as $result ) :
			$request_message = [];
